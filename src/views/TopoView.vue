        ...
        <div class="main-content">
            ...
            <div class="node-palette">
                ...
                <div class="palette-items">
                    <div v-for="nodeType in nodeTypes" :key="nodeType.id" class="palette-item" draggable="true"
                        @dragstart="handleDragStart(nodeType)" @dragend="handleDragEnd">
                        <div class="node-preview" :style="{
            ...
            <!-- 拓扑图区域 -->
            <div ref="dropZone" class="topology-dropzone" @dragover.prevent @drop="handleDrop">
            ...
            <div v-if="selectedElement">
                ...
                <div class="property-group">
                    ...
                    <div class="position-controls">
                        <input type="number" v-model="selectedElement.style.x" @change="updatePosition" />
                        <input type="number" v-model="selectedElement.style.y" @change="updatePosition" />
                    </div>
                </div>
            </div>
            <div v-else class="empty-state">请选择一个节点或连线进行编辑</div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        updateElementType() {
            ...
            const nodeType = this.nodeTypes.find(
                (t) => t.id === this.selectedElement.data.type
            );
            if (nodeType) {
        ...
        updatePosition() {
            ...
            const snapX = this.enableSnapToGrid
                ? Math.round(this.selectedElement.style.x / this.gridSize) *
                this.gridSize
                : this.selectedElement.style.x;
            const snapY = this.enableSnapToGrid
                ? Math.round(this.selectedElement.style.y / this.gridSize) *
                this.gridSize
        ...
        bindEvents() {
            ...
            this.graph.on("node:dragend", (event) => {
                ...
                if (this.enableSnapToGrid) {
                    const snapX =
                        Math.round(node.style.x / this.gridSize) * this.gridSize;
                    const snapY =
                        Math.round(node.style.y / this.gridSize) * this.gridSize;
